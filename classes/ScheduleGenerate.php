...
class ScheduleGenerate {
	...
	protected $isScheduleExist = false;
	protected $existScheduleCount = 0;
	...
	public function makeDataForCalendarAndList($schedules){
		...
		foreach($schedules as $a){
			...
			//var_dump($num);
			if($this->ingest->getClassCount() == $a->getNumClasses()){
			...
			$timeSlots = ["8:30 am", "9:30 am", "10:30 am", "11:30 am", "12:30 pm"
			...
			foreach($timeArray as $k2 => $v){
				...
				for($i = $a->getFirstTime()[0]; $i < (5 + $a->getFirstTime()[0]); $i++){
					//var_dump($a->getFirstTime());
					/** @var array|\Section $v */
			...
			foreach($rows as $row){
				if($row["timestamp"] == "8:30 am" || $row["timestamp"] == "9:30 am" || $row["timestamp"] == "10:30 am" || $row["timestamp"] == "11:30 am"){
					continue;
				}else{
					$afterBreak = $row["timestamp"];
					break;
				}
			}
			
			if($this->ingest->getClassCount() == $a->getNumClasses()){
			$weekSchedule[intval($num / 2)][] = ["rows" => $rows,  "afterBreak" => $afterBreak, "daysString" => $daysString,
				"hasAllClasses" => $this->ingest->getClassCount() == $a->getNumClasses(),
				"numUnits" => ["num" => $a->getNumUnits(), "string" => $this->plural("unit", $a->getNumUnits())],
				"numClasses" => ["num" => $a->getNumClasses(), "string" => $this->plural("class", $a->getNumClasses())],
				"numCPD" => ["num" => reset($cpd), "string" => $this->plural("class", reset($cpd))],
	...
	public function getIsScheduleExist(){
		return $this->isScheduleExist;
	}

	public function getExistScheduleCount(){
		return $this->existScheduleCount;
	}

	public function setIsScheduleExist($a){
		$this->isScheduleExist = $a;
	}
